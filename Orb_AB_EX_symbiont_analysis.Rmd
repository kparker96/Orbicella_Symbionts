...
qpcr_MIA1_Ofav <- MIA1 %>%
  filter(Species == "Ofav") %>%
  select(Sample.Name = SampleName, File.Name, depth = Depth, A.CT.mean, B.CT.mean, C.CT.mean, D.CT.mean,
...
```{r}
Ofav_qpcr <- rbind(qpcr_EX_Ofav, qpcr_AB_Ofav, qpcr_MIA1_Ofav)
```
...
```{r}
Ofav<- rbind(EXAB_Ofav, KEY_Ofav, CUR_Ofav) %>%
