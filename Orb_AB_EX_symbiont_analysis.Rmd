...
#Import Abaco Depth Data and add to qpcr data
```{r}
#library(readr)
...
```{r}
Ofav<- rbind(EXAB_Ofav, KEY_filter_Ofav) %>%
...
CUR_Ofav <- GRE %>%
  ...
  mutate(dom_sym = case_when(Breviolum > Cladocopium & Breviolum > Gerakladium & Breviolum > Symbiodinium
                             ~ "Breviolum",
                             Cladocopium > Breviolum & Cladocopium > Gerakladium & Cladocopium > Symbiodinium
                             ~ "Cladocopium",
                             Gerakladium > Breviolum & Gerakladium > Cladocopium & Gerakladium > Symbiodinium
                             ~ "Gerakladium",
                             Symbiodinium > Breviolum & Symbiodinium > Cladocopium & Symbiodinium >
                             Gerakladium ~ "Symbiodinium"))

CUR_symcomp <- CUR_Ofav %>%
  select(Sample.Name, Symbiodinium, Breviolum, Cladocopium, Gerakladium) %>%
  gather(key = "sym", value = "value", -Sample.Name) 


ggplot(CUR_symcomp, aes(x = Sample.Name, y = value, fill = sym)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = c('#a6cee3','#1f78b4','#b2df8a','#33a02c'))
