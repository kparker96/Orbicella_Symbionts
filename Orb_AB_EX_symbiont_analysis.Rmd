...
# Import Florida Keys Data and combine with Exumas and Abaco Data
```{r}
KEY_Ofav <- readxl::read_xlsx(path = "Data/qPCR_KEY/qPCR_FL_Keys_2015-2016.xlsx") %>%
  select(Sample.Name, depth, Symbiodinium, Breviolum, Cladocopium, Durusdinium) 
  
Ofav<- rbind(Ofav_symname, KEY_Ofav) %>%
...
>>>>>>> 73004340b30c30f0b10ec6dd42f7dada5f227344
=======
<<<<<<< HEAD
---
title: "Orb_EX_AB_sym_analysis"
...
Ofav<- rbind(Ofav_symname, KEY_rbind) %>%
  mutate(dom_sym = case_when(Breviolum > Cladocopium & Breviolum > Durusdinium & Breviolum > Symbiodinium
                             ~ "Breviolum",
                             Cladocopium > Breviolum & Cladocopium > Durusdinium & Cladocopium > Symbiodinium
                             ~ "Cladocopium",
...
edges_orb <- net_data_orb %>%
  gather(key = "Genera", value = "weight", -Sample.Name) %>%
  filter(weight != 0) %>%
  mutate(weight = weight^(1/2))  # use exponent >1 to make rel. abund. matter more; <1 to matter less (i.e., pres/abs)
...
plot(net_orb, edge.curved = 0,
     edge.width = 5 * (E(net_orb)$weight)^0.5, 
     edge.color = "gray65",
     edge.curved = 1,
     vertex.label = V(net_orb)$lab,
     vertex.label.font = 2,
     vertex.label.cex = 2.5,
     vertex.label.color = "black", 
     vertex.size = ifelse(grepl("EX|MR|SC", names(V(net_orb))), 10, 15),
     vertex.color = ifelse(grepl("EX|MR|SC", names(V(net_orb))), 
                           colorRampPalette(c("white", "black"))(100)[as.numeric(cut(V(net_orb)$depth,breaks = 100))],
                           c('#a6cee3','#1f78b4','#b2df8a','#33a02c')))
```
...
>>>>>>> 73004340b30c30f0b10ec6dd42f7dada5f227344
>>>>>>> e35e6a9b260cba0f2c0c43f4e13b82d193496aa4
