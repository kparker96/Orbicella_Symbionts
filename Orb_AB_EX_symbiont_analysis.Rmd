...
sample_data_EX <- raw_sample_data_EX %>%
  select(Sample.Name = sample_id, site, genus, species, depth = colony_depth_m, site, range)
...
qpcr_EX_Ofav <- EX_Ofav %>%
  select(Sample.Name, File.Name, site, depth, A.CT.mean, B.CT.mean, C.CT.mean, D.CT.mean, A.CT.sd, B.CT.sd,
         C.CT.sd,D.CT.sd, A.reps, B.reps, C.reps, D.reps, A.A) %>%
...
MIA1_Ofav <- MIA1 %>%
  filter(Species == "Ofav") %>%
  select(Sample.Name = SampleName, site = Reef, depth = depth_m, A.CT.mean, B.CT.mean, C.CT.mean, D.CT.mean,
...
MIA2_Ofav <- MIA2 %>% 
 filter(Species == "Ofav") %>%
  select(Sample.Name = SampleName, site = Location, A.CT.mean, B.CT.mean, C.CT.mean, D.CT.mean,
  A.CT.sd,B.CT.sd, C.CT.sd, D.CT.sd, A.reps, B.reps, C.reps, D.reps) %>%
  mutate(A.CT.mean = as.numeric(A.CT.mean),
         B.CT.mean = as.numeric(B.CT.mean),
         C.CT.mean = as.numeric(C.CT.mean),
...
MIA_Ofav <- MIA_qpcr_prop %>%
  select(Sample.Name, region, site, depth, Symbiodinium = A.prop, Breviolum = B.prop, Cladocopium = C.prop, Durusdinium = D.prop) %>%
  mutate(depth = NA)
