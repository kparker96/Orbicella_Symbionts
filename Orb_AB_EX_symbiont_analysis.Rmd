...
matrix <- symbiont_pres %>%
  mutate(AA = ifelse(A > 0 & B == 0 & C == 0 & D == 0, 1, 0),
         ...
         ABC = ifelse(A > 0 & B > 0 & C > 0, 1, 0),
         ABD = ifelse(A > 0 & B > 0 & D > 0, 1, 0),
         ACD = ifelse(A > 0 & C > 0 & D > 0, 1, 0),
         BCD = ifelse(B > 0 & C > 0 & D > 0, 1, 0),
         ABCD = ifelse( A > 0 & B > 0 & C > 0 & D > 0, 1, 0)) 



sums <- matrix %>%
  select(A = AA, B = BB, C = CC, D = DD, AB, AC, AD, BC, BD, CD, ABC, ACD, BCD, ABCD) %>%
  colSums()
