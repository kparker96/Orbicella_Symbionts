...
```{r}
KEY_Ofav <- readxl::read_xlsx(path = "Data/qPCR_KEY/qPCR_Raw_Data_FL_Keys_2015-2016.xlsx") 

# Filter out only Healthy samples
KEY_healthy <- KEY_Ofav %>%
  filter(condition == "H")

# Which Healthy samples have the same tag number
KEY_dupe_tags <- KEY_healthy %>%
  group_by(tag_number) %>%
  count(vars = "tag_number") %>%
  filter(n > 1) 

# All data for healthy samples with same tag number 
KEY_tags <- KEY_healthy %>%
  filter(tag_number %in% KEY_dupe_tags$tag_number) %>%
  arrange(tag_number)

# Gather data for figure
key_both <- KEY_tags %>%
  select(tag_number, time_point, Symbiodinium, Breviolum, Cladocopium, Durusdinium) %>%
  gather(key = sym, value = value, -tag_number, -time_point)

# Symbiont composition per sample, grouped by final and inital sampling timepoint 
ggplot(key_both, aes(x = factor(tag_number), y = value, fill = sym)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = c('#a6cee3','#1f78b4','#b2df8a','#33a02c')) +
  facet_wrap(~time_point)

ggsave(filename = "florida_keys.png", width = 350, height = 175, units = "mm")

# Healthy corals in format to combine with other data sets. STILL CONTAINS MULTIPLE TAG # SAMPLES
KEY_filter_Ofav <- KEY_healthy %>% 
...
```{r}
Ofav<- rbind(EXAB_Ofav, KEY_filter_Ofav, CUR_Ofav) %>%
