...
qpcr_karp <- rbind(MIA1_Ofav, MIA2_Ofav) %>%
  mutate(region = "Miami") %>%
  rbind(DRT) %>%
  select(Sample.Name, region, site, depth, A.CT.mean, B.CT.mean, C.CT.mean, D.CT.mean, A.CT.sd,B.CT.sd, C.CT.sd, D.CT.sd, A.reps, B.reps, C.reps, D.reps ) %>%
      mutate(A.CT.mean = as.numeric(A.CT.mean),
...
qpcr_williamson <- rbind(WEL, WER) %>%
  mutate(A.CT.sd = 0, B.CT.sd = 0, C.CT.sd = 0, D.CT.sd = 0)

```

```{r}
need_to_clean <- rbind(qpcr_AB, qpcr_karp, qpcr_williamson) %>%
...
ELU <- qpcr_prop %>%
  ...
  select(Sample.Name, site, region, Symbiodinium, Breviolum, Cladocopium, Durusdinium)
```
...
```{r}
Ofav<- rbind(AB_Ofav, EX_Ofav, KEY_Ofav, CUR_Ofav, MIA_Ofav, DRT_Ofav) %>%
